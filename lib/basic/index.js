...
class basic {
    ...
    * */
    insertOne(object, collection, db){
        if (this.mode == "transaction") {
            ...
            return `client.db("${db?db:this.db}").collection("${collection}").insertOne(${JSON.stringify(object)}, { "session": session });`;
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(object) != "[object Object]") {
    ...
    * */
    insertMany(array, collection, db){
        if (this.mode == "transaction") {
            ...
            return `client.db("${db?db:this.db}").collection("${collection}").insertMany(${JSON.stringify(array)}, { "session": session });`;
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(array) != "[object Array]") {
    ...
    * */
    deleteOne(object, collection, db){
        if (this.mode == "transaction") {
            ...
            return `client.db("${db?db:this.db}").collection("${collection}").deleteOne(${JSON.stringify(object)}, { "session": session });`;
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(object) != "[object Object]") {
                    return reject(new Error("the first param should be Object"));
    ...
    * */
    deleteMany(object, collection, db){
        if (this.mode == "transaction") {
            ...
            return `client.db("${db?db:this.db}").collection("${collection}").deleteMany(${JSON.stringify(object)}, { "session": session });`;
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(object) != "[object Object]") {
                    return reject(new Error("the first param should be Object"));
    ...
    * */
    find(object={}, collection, options={}, db){
        if (this.mode == "transaction") {
            ...
            // TODO
            return `client.db("${db?db:this.db}").collection("${collection}").find(${JSON.stringify(object)}, { "session": session }).limit(${_limit}).skip(${_skip}).sort(${_sort}).toArray()`;
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(object) != "[object Object]") {
                    return reject(new Error("the first param should be Object"));
    ...
    findOne(object={}, collection, options={}, db){
        if (this.mode == "transaction") {
            ...
            if (Object.keys(options).length == 0) {
                return `client.db("${db?db:this.db}").collection("${collection}").findOne(${JSON.stringify(object)}, { "session": session });`;
            } else {
                ...
                return `client.db("${db?db:this.db}").collection("${collection}").findOne(${JSON.stringify(object)}, ${options_str});`;
            }
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(object) != "[object Object]") {
    ...
    * */
    findOneAndUpdate(object={}, doc, collection, options={}, db){
        if (this.mode == "transaction") {
            ...
            if (Object.keys(options).length == 0) {
                return `client.db("${db?db:this.db}").collection("${collection}").findOneAndUpdate(${JSON.stringify(object)}, ${JSON.stringify(doc)}, { "session": session });`;
            } else {
                options["nayo_replace"] = "nayo_replace";
                let options_str = JSON.stringify(options).replace('"nayo_replace":"nayo_replace"', '"session": session');
                return `client.db("${db?db:this.db}").collection("${collection}").findOneAndUpdate(${JSON.stringify(object)}, ${JSON.stringify(doc)}, ${options_str});`;
            }
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(object) != "[object Object]") {
    ...
    * */
    findOneAndDelete(object={}, collection, options={}, db){
        if (this.mode == "transaction") {
            ...
            if (Object.keys(options).length == 0) {
                return `client.db("${db?db:this.db}").collection("${collection}").findOneAndDelete(${JSON.stringify(object)}, { "session": session });`;
            } else {
                options["nayo_replace"] = "nayo_replace";
                let options_str = JSON.stringify(options).replace('"nayo_replace":"nayo_replace"', '"session": session');
                return `client.db("${db?db:this.db}").collection("${collection}").findOneAndDelete(${JSON.stringify(object)}, ${options_str});`;
            }
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(object) != "[object Object]") {
    ...
    * */
    findOneAndReplace(object={}, doc, collection, options={}, db){
        if (this.mode == "transaction") {
            ...
            if (Object.keys(options).length == 0) {
                return `client.db("${db?db:this.db}").collection("${collection}").findOneAndReplace(${JSON.stringify(object)}, ${JSON.stringify(doc)}, { "session": session });`;
            } else {
                options["nayo_replace"] = "nayo_replace";
                let options_str = JSON.stringify(options).replace('"nayo_replace":"nayo_replace"', '"session": session');
                return `client.db("${db?db:this.db}").collection("${collection}").findOneAndReplace(${JSON.stringify(object)}, ${JSON.stringify(doc)}, ${options_str});`;
            }
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(object) != "[object Object]") {
    ...
    *
    * */
    aggregate(pipline, collection, options={}, db) {
        if (this.mode == "transaction") {
            ...
            if (Object.keys(options).length == 0) {
                return `client.db("${db?db:this.db}").collection("${collection}").aggregate(${JSON.stringify(pipline)}, { "session": session });`;
            } else {
                options["nayo_replace"] = "nayo_replace";
                let options_str = JSON.stringify(options).replace('"nayo_replace":"nayo_replace"', '"session": session');
                return `client.db("${db?db:this.db}").collection("${collection}").aggregate(${JSON.stringify(pipline)}, ${options_str});`;
            }
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(pipline) != "[object Array]") {
                ...
                try {
                    let client = await this.client;
                    let connect = client.db(this.db).collection(collection);
                    let _promise_ret = await connect.aggregate(pipline, options).toArray();
                    this.process_time = _timer.end();
                    query_done(this.config.logging, this.process_time, this.query_statement);
    ...
    * */
    updateMany(filter, doc, collection, options={}, db){
        if (this.mode == "transaction") {
            ...
            if (Object.keys(options).length == 0) {
                return `client.db("${db?db:this.db}").collection("${collection}").updateMany(${JSON.stringify(filter)}, ${JSON.stringify(doc)}, { "session": session });`;
            } else {
                options["nayo_replace"] = "nayo_replace";
                let options_str = JSON.stringify(options).replace('"nayo_replace":"nayo_replace"', '"session": session');
                return `client.db("${db?db:this.db}").collection("${collection}").updateMany(${JSON.stringify(filter)}, ${JSON.stringify(doc)}, ${options_str});`;
            }
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(filter) != "[object Object]") {
