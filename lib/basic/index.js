...
class basic {
    ...
    * @params: db_name
    * */
    async _check_database(client, db) {
        let _database_info = await client.db(client.s.options.authSource).admin().listDatabases({
            nameOnly: true
        });
        let _database_list = _database_info["databases"];
        let _available_database_name = [];
        for (let _item of _database_list) {
            _available_database_name.push(_item.name);
        }
        if (_available_database_name.includes(db)) {
            return true;
        } else {
            return false;
        }
    ...
    * @params: client
    * */
    async _get_available_database(client) {
        let _database_info = await client.db(client.s.options.authSource).admin().listDatabases({
            nameOnly: true
        });
        let _database_list = _database_info["databases"];
        ...
        return _available_database_name;
    }
    ...
    * */
    ...
    async _do_in_session(client, array, available_db, session){
        let _db_reg = /^client.db\("(\S*)"\).collection/;
        session.startTransaction(this.config.transaction);
        for (let query of array) {
            if (available_db.includes(query.match(_db_reg)[1])) {
                await this.__query_wrap(query, client, session);
            } else {
                throw new Error(`db [${query.match(_db_reg)[1]}] is not available`);
                break;
            }
        }
        return session;
    ...
    transaction(array){
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                let _available_database = await this._get_available_database(client);
                // ===get new session================
                let _session = client.startSession();
                try {
                    // ===do in the session===============
                    _session = await this._do_in_session(client, array, _available_database, _session);
                    // ===commit session================
    ...
    * */
    insertOne(object, collection, db){
        if (this.mode == "transaction") {
            ...
            if (Object.prototype.toString.call(db) != "[object String]") {
                throw new Error("the db param should be String");
            }
            return `client.db("${db}").collection("${collection}").insertOne(${JSON.stringify(object)}, { "session": session });`;
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(object) != "[object Object]") {
                ...
                if (Object.prototype.toString.call(db) != "[object String]") {
                    return reject(new Error("the db param should be String"));
                }
                let _timer = new Timer_Record();
                _timer.start();
                try {
                    let client = await this.client;
                    if (await this._check_database(client, db)) {
                        let connect = client.db(db).collection(collection);
                        ...
                        _timer.reset();
                        reject(new Error(`db [${db}] is not available`));
                    }
                } catch (e) {
                    _timer.reset();
                    reject(e);
    ...
    * */
    insertMany(array, collection, db){
        if (this.mode == "transaction") {
            ...
            return `client.db("${db}").collection("${collection}").insertMany(${JSON.stringify(array)}, { "session": session });`;
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(array) != "[object Array]") {
    ...
    * */
    deleteOne(object, collection, db){
        if (this.mode == "transaction") {
            ...
            return `client.db("${db}").collection("${collection}").deleteOne(${JSON.stringify(object)}, { "session": session });`;
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(object) != "[object Object]") {
                    return reject(new Error("the first param should be Object"));
    ...
    * */
    deleteMany(object, collection, db){
        if (this.mode == "transaction") {
            ...
            return `client.db("${db}").collection("${collection}").deleteMany(${JSON.stringify(object)}, { "session": session });`;
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(object) != "[object Object]") {
                    return reject(new Error("the first param should be Object"));
    ...
    * */
    find(object, collection, options, db){
        if (this.mode == "transaction") {
            ...
            // TODO
            return `client.db("${db}").collection("${collection}").find(${JSON.stringify(object)}, { "session": session }).limit(${_limit}).skip(${_skip}).sort(${_sort}).toArray()`;
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(object) != "[object Object]") {
                    return reject(new Error("the first param should be Object"));
    ...
    findOne(object, collection, options, db){
        if (this.mode == "transaction") {
            ...
            if (Object.keys(options).length == 0) {
                return `client.db("${db}").collection("${collection}").findOne(${JSON.stringify(object)}, { "session": session });`;
            } else {
                ...
                return `client.db("${db}").collection("${collection}").findOne(${JSON.stringify(object)}, ${options_str});`;
            }
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(object) != "[object Object]") {
    ...
    * */
    findOneAndUpdate(object, doc, collection, options, db){
        if (this.mode == "transaction") {
            ...
            if (Object.keys(options).length == 0) {
                return `client.db("${db}").collection("${collection}").findOneAndUpdate(${JSON.stringify(object)}, ${JSON.stringify(doc)}, { "session": session });`;
            } else {
                options["nayo_replace"] = "nayo_replace";
                let options_str = JSON.stringify(options).replace('"nayo_replace":"nayo_replace"', '"session": session');
                return `client.db("${db}").collection("${collection}").findOneAndUpdate(${JSON.stringify(object)}, ${JSON.stringify(doc)}, ${options_str});`;
            }
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(object) != "[object Object]") {
    ...
    * */
    findOneAndDelete(object, collection, options, db){
        if (this.mode == "transaction") {
            ...
            if (Object.keys(options).length == 0) {
                return `client.db("${db}").collection("${collection}").findOneAndDelete(${JSON.stringify(object)}, { "session": session });`;
            } else {
                options["nayo_replace"] = "nayo_replace";
                let options_str = JSON.stringify(options).replace('"nayo_replace":"nayo_replace"', '"session": session');
                return `client.db("${db}").collection("${collection}").findOneAndDelete(${JSON.stringify(object)}, ${options_str});`;
            }
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(object) != "[object Object]") {
    ...
    * */
    findOneAndReplace(object, doc, collection, options, db){
        if (this.mode == "transaction") {
            ...
            if (Object.keys(options).length == 0) {
                return `client.db("${db}").collection("${collection}").findOneAndReplace(${JSON.stringify(object)}, ${JSON.stringify(doc)}, { "session": session });`;
            } else {
                options["nayo_replace"] = "nayo_replace";
                let options_str = JSON.stringify(options).replace('"nayo_replace":"nayo_replace"', '"session": session');
                return `client.db("${db}").collection("${collection}").findOneAndReplace(${JSON.stringify(object)}, ${JSON.stringify(doc)}, ${options_str});`;
            }
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(object) != "[object Object]") {
    ...
    * */
    aggregate(pipeline, collection, options, db) {
        if (this.mode == "transaction") {
            ...
            if (Object.keys(options).length == 0) {
                return `client.db("${db}").collection("${collection}").aggregate(${JSON.stringify(pipeline)}, { "session": session });`;
            } else {
                options["nayo_replace"] = "nayo_replace";
                let options_str = JSON.stringify(options).replace('"nayo_replace":"nayo_replace"', '"session": session');
                return `client.db("${db}").collection("${collection}").aggregate(${JSON.stringify(pipeline)}, ${options_str});`;
            }
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(pipeline) != "[object Array]") {
    ...
    * */
    updateMany(filter, doc, collection, options, db){
        if (this.mode == "transaction") {
            ...
            if (Object.keys(options).length == 0) {
                return `client.db("${db}").collection("${collection}").updateMany(${JSON.stringify(filter)}, ${JSON.stringify(doc)}, { "session": session });`;
            } else {
                options["nayo_replace"] = "nayo_replace";
                let options_str = JSON.stringify(options).replace('"nayo_replace":"nayo_replace"', '"session": session');
                return `client.db("${db}").collection("${collection}").updateMany(${JSON.stringify(filter)}, ${JSON.stringify(doc)}, ${options_str});`;
            }
        } else {
            return new Promise(async (resolve, reject) => {
                if (Object.prototype.toString.call(filter) != "[object Object]") {
