...
// is_pass help function
function __gen_pipeline_work_for_is_pass(work_item, index) {
    return [
        {
            $lookup: {
                from: work_item.collection,
                pipeline: [
                    {
                        $match: work_item.target_doc
                    },
                    {
                        $limit: 1
                    }
...
function ___method_behaviour(work_pack, index) {
    let _target_item;
    let _param;
    let _work_item;
    let _sentence;
...
function ___gen_connect_list(work_pack, db_name, index) {
    return `client.db("${db_name}").collection("${work_pack.collection}");`;
}

function __convert_work(work_list, dbname) {
    let _list_connect = [];
    let _list_do = [];
...
class Management {
    constructor(options) {
        ...
        this.worker = new Worker(this.db_url, this.db_name);
        event.on("accept", (bag) => {
